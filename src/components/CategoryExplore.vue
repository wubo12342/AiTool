  ...
  <div class="min-h-screen pb-20 animate-in fade-in duration-500 relative overflow-hidden">
    <div class="relative z-8">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <RouterLink 
          to="/" 
          class="inline-flex items-center gap-2 text-slate-500 hover:text-primary transition-colors mb-8 no-underline text-sm font-medium"
        >
        ...
        <div v-if="category" class="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <h1 class="text-4xl md:text-5xl font-bold mb-4 text-slate-900">{{ category.name }}</h1>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 relative z-20">
      <!-- Toolbar -->
      <div class="bg-white rounded-2xl shadow-xl p-2 mb-12 border border-slate-100 flex items-center gap-2">
        <div class="relative flex-1">
          ...
          <input 
            ...
            class="w-full pl-12 pr-4 py-4 bg-slate-50 border-none rounded-xl focus:ring-2 focus:ring-primary/20 outline-none transition-all"
          >
        </div>
      ...
      <!-- Error State -->
      <div v-else-if="error" class="py-32 text-center bg-white rounded-3xl border border-dashed border-slate-200">
      ...
      <!-- Empty State -->
      <div v-else-if="tools.length === 0" class="py-32 text-center bg-white rounded-3xl border border-dashed border-slate-200">
        <div class="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-6">
          <LayoutGrid class="w-10 h-10 text-slate-300" />
        </div>
        <h3 class="text-xl font-bold text-slate-900 mb-2">此分類暫無工具</h3>
        <p class="text-slate-500">我們正在持續收錄更多優秀的 AI 工具，敬請期待。</p>
      </div>
